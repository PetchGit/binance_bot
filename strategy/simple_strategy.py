...
class TradingStrategy:
    def __init__(self, strength_lookback,holding_period):
    ...
    async def on_candle_closed(self, market_data, orderbook_data, portfolio_data, liquidation_data, order_history, trade_history):
        if self.count == 0:
            ...
            self.count += 1
<<<<<<< HEAD
        elif self.count > self.holding_period:
            self.count = 0
        else:
            self.count += 1
=======
...
>>>>>>> b8600444a5eb8bdfdc02648c9b9a7e23490657fb
    ...
    def add_indicators(self, df):
        indicator_class_ema = EMAIndicator(df['close'], window = 14)
        df['ema'] = indicator_class_ema.ema_indicator()
        return df
    ...
    async def momentum_entry(self, market_data):
        ...
        self.is_momentum_time = not(self.is_momentum_time)
